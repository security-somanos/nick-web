...
export default function VideoBoxesSection() {
  ...
  // Control audio for the currently hovered video
  useEffect(() => {
    videoRefs.current.forEach((video, index) => {
      if (video) {
        const isCurrentlyHovered = hoveredBox === videoBoxes[index].id
        // Only unmute if this video is hovered AND audio is enabled
        video.muted = !(isCurrentlyHovered && audioEnabled)
      }
    })
  }, [hoveredBox, audioEnabled])
      ...
      <div className="w-full mx-auto px-0">
           ...
           {videoBoxes.map((box, index) => (
             ...
             >
                 ...
                 className="shadow-md shadow-black/70 border border-[#252525] overflow-hidden rounded-12 cursor-pointer h-[200px] w-full relative block group"
               ...
               >
                 <div className="relative h-full z-10 m-[1px] rounded-12 overflow-hidden bg-fill-glass-secondary backdrop-blur-2xl">
                   <div className="bg-[#1a1a1a] relative overflow-hidden rounded-12 h-full">
                     <div style={{ position: "relative", height: "100%" }}>
                                             {/* Video Background */}
                      <div
                        className={`absolute right-0 left-0 top-0 w-full h-[calc(100%-40px)] transition-all duration-300 ${
                      ...
                      >
                        {loadedVideos.has(box.id) ? (
                          <video
                            ...
                            loop
                            playsInline
                            muted={!(hoveredBox === box.id && audioEnabled)}
                            className="h-full w-full object-cover object-bottom brightness-75 transition-opacity duration-300 opacity-0 group-hover:opacity-100"
                          >
                        ...
                        <div
                          className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent transition-opacity duration-300 opacity-0 group-hover:opacity-100"
                        ></div>
                      </div>
 
                                             {/* Content (Title, etc.) - Always visible */}
                      <div
                        className="relative z-[1] h-full flex flex-col justify-center items-center font-mono uppercase"
                      >
                       ...
                       {hoveredBox === box.id && (
                         <button
                           ...
                           className="absolute top-3 right-3 z-50 w-8 h-8 bg-black/60 hover:bg-black/80 border border-white/30 rounded-full flex items-center justify-center transition-colors"
                         >
                           {audioEnabled ? (
            ...
            <div className="mt-2 text-start me-auto px-4">
